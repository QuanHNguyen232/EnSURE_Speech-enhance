...
if opt.generator_type == 'unet': # not check if it runs
    G_AB = UnetGenerator(input_nc=input_nc, output_nc=1, num_downs=7, use_dropout=False).to(device)
    G_BA = UnetGenerator(input_nc=input_nc, output_nc=1, num_downs=7, use_dropout=False).to(device)
...
if opt.pretrain_dir is not None:
    ...
    try:
        state_dict_G_AB = torch.load(os.path.join(opt.pretrain_dir, 'G_AB.pt'))
        state_dict_G_BA = torch.load(os.path.join(opt.pretrain_dir, 'G_BA.pt'))
        state_dict_D_A = torch.load(os.path.join(opt.pretrain_dir, 'D_A.pt'))
        state_dict_D_B = torch.load(os.path.join(opt.pretrain_dir, 'D_B.pt'))
        ...
        try:
            new_state_dict_G_AB = OrderedDict()
            for k, v in state_dict_G_AB.items(): new_state_dict_G_AB[k[7:]] = v # k[7:] = remove "module."
            G_AB.load_state_dict(new_state_dict_G_AB)

            new_state_dict_G_BA = OrderedDict()
            for k, v in state_dict_G_BA.items(): new_state_dict_G_BA[k[7:]] = v # k[7:] = remove "module."
            G_BA.load_state_dict(new_state_dict_G_BA)

            new_state_dict_D_A = OrderedDict()
            for k, v in state_dict_D_A.items(): new_state_dict_D_A[k[7:]] = v # k[7:] = remove "module."
            D_A.load_state_dict(new_state_dict_D_A)

            new_state_dict_D_B = OrderedDict()
            for k, v in state_dict_D_B.items(): new_state_dict_D_B[k[7:]] = v # k[7:] = remove "module."
            D_B.load_state_dict(new_state_dict_D_B)
...
if opt.use_DDP:
    ...
    G_AB = DDP(G_AB, device_ids=[ddp_local_rank], output_device=ddp_local_rank, broadcast_buffers=False)
    G_BA = DDP(G_BA, device_ids=[ddp_local_rank], output_device=ddp_local_rank, broadcast_buffers=False)
    D_A = DDP(D_A, device_ids=[ddp_local_rank], output_device=ddp_local_rank, broadcast_buffers=False)
    D_B = DDP(D_B, device_ids=[ddp_local_rank], output_device=ddp_local_rank, broadcast_buffers=False)
...
scheduler_D_A = torch.optim.lr_scheduler.MultiStepLR(optimizer_D_A, milestones=milestones, gamma=opt.gamma)
scheduler_D_B = torch.optim.lr_scheduler.MultiStepLR(optimizer_D_B, milestones=milestones, gamma=opt.gamma)
...
def sample_image(steps_done):
    ...
    with torch.no_grad():
        ...
        out_B = G_AB(inp_A, emb_A).squeeze(0) # remove batch
...
with torch.autograd.set_detect_anomaly(True):
    ...
    for epoch in range(n_epochs):
        ...
        for batch in train_progress_bar:
            ...
            # forward G_AB
            fake_B_spec = forward_G(G_AB, real_A_spec, real_A_wav)
            # forward G_BA
            fake_A_spec = forward_G(G_BA, real_B_spec, real_B_wav)
            
            # update D_A
            set_requires_grad(D_A, True)
            optimizer_D_A.zero_grad()
            G_loss += backward_D(D_A, real_A_spec, real_B_spec, fake_B_spec)
            optimizer_D_A.step()
            # update D_B
            set_requires_grad(D_B, True)
            optimizer_D_B.zero_grad()
            G_loss += backward_D(D_B, real_B_spec, real_A_spec, fake_A_spec)
            optimizer_D_B.step()

            # update G_AB and G_BA
            set_requires_grad(D_A, False) # D requires no gradients when optimizing G
            set_requires_grad(D_B, False)
            optimizer_G.zero_grad()
            G_loss += backward_G(D_A, real_A_spec, real_B_spec, fake_B_spec)
            G_loss += backward_G(D_B, real_B_spec, real_A_spec, fake_A_spec)
            optimizer_G.step()
            ...
            if master_process:
                if curr_step % opt.sample_interval == 0:
                    ...
                    torch.save(G_AB.state_dict(), f'./{ckpt_dir}/G_AB.pt')
                    torch.save(G_BA.state_dict(), f'./{ckpt_dir}/G_BA.pt')
                    torch.save(D_A.state_dict(), f'./{ckpt_dir}/D_A.pt')
                    torch.save(D_B.state_dict(), f'./{ckpt_dir}/D_B.pt')
