...
def freeze_model(m, layer=None):
  for name, p in m.named_parameters():
    p.requires_grad = False
    if layer is not None and layer in name:
...
class ContextEncoder(nn.Module):
    output_size = (203, 1024)
    def __init__(self, device, isFreeze=True) -> None:
        super(ContextEncoder, self).__init__()
        self.device = device
        self.isFreeze = isFreeze
        self.encoder = self.get_context_encoder()

    def forward(self, x):
        # since all wavs are padded, wav_lens is all ones
        # output shape = (batch_size, 203, 1024)
        wav_lens = torch.ones(x.shape[0]).to(self.device)
        return self.encoder(x, wav_lens)
    ...
    def get_context_encoder(self):
        ...
        if self.isFreeze:
            return encoder.apply(lambda x: freeze_model(x, layer='enc.linear_0'))
        return encoder
    ...
    # wavs, wav_lens = batch, rel_length
    # encoder = dict(asr_model.named_modules())['mods.encoder']
    # with torch.no_grad():
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc=1, output_nc=1, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model = [nn.ReflectionPad2d(3),
                 ...
                 norm_layer(ngf),
                 nn.ReLU(True)]
        self.inputprocess = nn.Sequential(*model)
        ...
        self.ResNetblocks = nn.Sequential(*model)
        ...
        self.upsampling = nn.Sequential(*model)

    def forward(self, input, emb=None):
        """Standard forward"""
        if emb is not None:
            input = self.merge_input_emb(input, emb)
        return self.upsampling(self.ResNetblocks(self.downsampling(self.inputprocess(input))))
    
    def merge_input_emb(self, input, emb):
        # emb.shape = (batch, emb_h, emb_w)
        # input.shape = (batch, c, inp_h, inp_w)
        batch_size, emb_h, emb_w = emb.shape
        inp_h, inp_w = input.shape[-2:]
        xtimes = int((emb_h*emb_w) // (inp_h*inp_w))
        # trim
        emb = emb.view(batch_size, -1)[..., :xtimes*(inp_h*inp_w)]
        # reshape to input shape
        emb = emb.view(batch_size, -1, inp_h, inp_w)
        # add to input
        input = torch.cat([input, emb], 1)
        assert input.shape[1] == self.input_nc, input.shape
        return input
